...
- 正文出现的每个 `[n]` 都必须在来源列表中存在同编号条目。
- 每个来源条目必须包含从 Research Context 的 `URL:` 行原样复制的完整 URL。
- 没有 URL 的材料不得分配引用编号，也不得在正文中引用。
- 输出前核对：正文引用编号集合必须是来源列表编号集合的子集。
</Citation Integrity Rules>
...
- 正文每个 `[n]` 必须能在最终来源列表中找到同编号及完整 URL；没有 URL 的来源不得保留编号。
- 输出前核对正文引用编号集合是来源列表编号集合的子集，发现悬空编号时必须删除该声明或补齐已有来源 URL。
- 输出语言与用户请求一致。
- 输出纯 Markdown，不要额外说明。
</Rules>
"""
