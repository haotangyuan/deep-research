...
|---|---|---|---:|---:|---|---|---|
| 参数与系统边界导致的证据冲突题 | MEDIUM | completed | 69.781 | 失败 | 报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失 | ReportAgent, ClaimVerifier, ClaimManifestPersistence | 为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。 |
| 参数与系统边界导致的证据冲突题 | HIGH | completed | 186.026 | 失败 | 报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失 | ReportAgent, ClaimVerifier, ClaimManifestPersistence | 为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。 |
| 参数与系统边界导致的证据冲突题 | ULTRA | failed | 1.796 | 失败 | ScopeAgent 产生了不必要或错误的澄清 | ScopeAgent | 把 Dataset 的 should_clarify 契约加入 Scope 提示词和回归测试；对信息已充分的问题禁止生成澄清，并记录触发澄清的具体缺口。 |
| 标准框架事实与落地映射题 | MEDIUM | completed | 84.625 | 失败 | 报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失 | ReportAgent, ClaimVerifier, ClaimManifestPersistence | 为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。 |
| 标准框架事实与落地映射题 | HIGH | completed | 118.057 | 失败 | 报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失 | ReportAgent, ClaimVerifier, ClaimManifestPersistence | 为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。 |
| 标准框架事实与落地映射题 | ULTRA | degraded | 215.650 | 通过 | 章节修订过程中丢失了较多 Claim | ReportSectionAgent, ReportConsistencyAgent | 修订提示词增加“保留已支持 Claim”约束；修订后按 claim_id 做差异检查，只有明确解决冲突或删除不支持内容时才允许 Claim 消失。 |

## 单 Case 根因证据

### 参数与系统边界导致的证据冲突题 / MEDIUM

- 功能状态：Scope=`effective`，ReportContent=`effective`，CitationPipeline=`ineffective`，Reviewer=`not_applicable`
- 质量：content=0.8786，evidence=0.3744，overall=0.6265
- `citation_claim_linkage_failure`（blocking，confidence=0.96）：报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失
  - Agent：ReportAgent, ClaimVerifier, ClaimManifestPersistence
  - 指标证据：hard_gate.failure_codes=['dangling_citation', 'unsupported_critical_claim']; claim_factuality=0.8; citation_traceability=0.0975609756097561
  - 优化建议：为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。

### 参数与系统边界导致的证据冲突题 / HIGH

- 功能状态：Scope=`effective`，ReportContent=`effective`，CitationPipeline=`ineffective`，Reviewer=`not_applicable`
- 质量：content=1.0，evidence=0.6875，overall=0.8438
- `citation_claim_linkage_failure`（blocking，confidence=0.96）：报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失
  - Agent：ReportAgent, ClaimVerifier, ClaimManifestPersistence
  - 指标证据：hard_gate.failure_codes=['dangling_citation', 'unsupported_critical_claim']; claim_factuality=1.0; citation_traceability=0.0
  - 优化建议：为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。

### 参数与系统边界导致的证据冲突题 / ULTRA

- 功能状态：Scope=`ineffective`，ReportContent=`degraded`，CitationPipeline=`not_evaluable`，Reviewer=`not_applicable`
- 质量：content=0.0，evidence=0.0，overall=0.0
- `scope_clarification_error`（blocking，confidence=0.99）：ScopeAgent 产生了不必要或错误的澄清
  - Agent：ScopeAgent
  - 指标证据：clarification_decision_accuracy=0.0; workflow_completed=0; run.outcome=hitl_wait
  - 优化建议：把 Dataset 的 should_clarify 契约加入 Scope 提示词和回归测试；对信息已充分的问题禁止生成澄清，并记录触发澄清的具体缺口。

### 标准框架事实与落地映射题 / MEDIUM

- 功能状态：Scope=`effective`，ReportContent=`effective`，CitationPipeline=`ineffective`，Reviewer=`not_applicable`
- 质量：content=0.9714，evidence=0.75，overall=0.8607
- `citation_claim_linkage_failure`（blocking，confidence=0.96）：报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失
  - Agent：ReportAgent, ClaimVerifier, ClaimManifestPersistence
  - 指标证据：hard_gate.failure_codes=['dangling_citation']; claim_factuality=1.0; citation_traceability=0.0
  - 优化建议：为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。

### 标准框架事实与落地映射题 / HIGH

- 功能状态：Scope=`effective`，ReportContent=`effective`，CitationPipeline=`ineffective`，Reviewer=`not_applicable`
- 质量：content=0.8929，evidence=0.6625，overall=0.7777
- `citation_claim_linkage_failure`（blocking，confidence=0.96）：报告内容基本正确，但引用与 Claim Manifest 的关联链路丢失
  - Agent：ReportAgent, ClaimVerifier, ClaimManifestPersistence
  - 指标证据：hard_gate.failure_codes=['dangling_citation', 'unsupported_critical_claim']; claim_factuality=1.0; citation_traceability=0.0
  - 优化建议：为每个关键 Claim 强制保存 citation_url、evidence_id 和报告引用标记的三方映射；在 ReportAgent 输出后运行一致性校验，存在悬空引用时先修复再完成报告。
...
- 功能状态：Scope=`effective`，ReportContent=`effective`，CitationPipeline=`effective`，Reviewer=`unproven`
- 质量：content=0.9，evidence=0.9875，overall=0.9438
- `section_revision_claim_loss`（warning，confidence=0.86）：章节修订过程中丢失了较多 Claim
  - Agent：ReportSectionAgent, ReportConsistencyAgent
  - 指标证据：claim_retention_after_revision=0.6587
  - 优化建议：修订提示词增加“保留已支持 Claim”约束；修订后按 claim_id 做差异检查，只有明确解决冲突或删除不支持内容时才允许 Claim 消失。
- `report_merge_information_loss`（warning，confidence=0.84）：章节合并阶段存在明显信息损失
  - Agent：ReportConsistencyAgent, ReportAgent
  - 指标证据：merge_information_loss=0.1875
  - 优化建议：合并前建立章节 Claim/Citation 清单，合并后逐项校验保留率；把去重和删除事实拆开，禁止因文本压缩丢失独有证据。
- `reviewer_uplift_not_measurable`（warning，confidence=1.00）：Reviewer 已消耗 Token，但缺少可比较的前后质量增益
  - Agent：UltraDynamicReviewer, RoundArtifactRecorder
  - 指标证据：reviewer_token_cost=6726.0; quality_delta_per_round=None; reviewer_consensus=split
  - 优化建议：冻结 Reviewer 前后的同口径报告/证据快照，并用同一 Judge 分别评分；只有得到 quality_delta_per_round 和 marginal_quality_per_1k_tokens，才能判断 Reviewer 是否值得保留。

## 同题档位决策

| 题目 | 比较 | Token 增量(k) | 质量增量 | Gate 增量 | 边际质量/1k Token | 决策 |
|---|---|---:|---:|---:|---:|---|
| 参数与系统边界导致的证据冲突题 | MEDIUM→HIGH | +116.245 | +0.2173 | +0 | +0.001869 | 质量有所提升但仍不可用；先修复共同 Gate 根因，再讨论升级 |
| 参数与系统边界导致的证据冲突题 | HIGH→ULTRA | -184.230 | -0.8438 | +0 | - | 不建议升级：高档位任务未完成 |
| 标准框架事实与落地映射题 | MEDIUM→HIGH | +33.432 | -0.0830 | +0 | -0.002483 | 不建议升级：增加成本但未改变 Hard Gate 结果 |
| 标准框架事实与落地映射题 | HIGH→ULTRA | +97.593 | +0.1661 | +1 | +0.001702 | 升级带来可用性提升；需结合新增 Token 判断是否只对该任务类型启用 |

## Agent 模块优化优先级

1. `ReportAgent`：关联 5 条根因诊断。
2. `ClaimVerifier`：关联 4 条根因诊断。
3. `ClaimManifestPersistence`：关联 4 条根因诊断。
4. `ReportConsistencyAgent`：关联 2 条根因诊断。
5. `ScopeAgent`：关联 1 条根因诊断。
6. `ReportSectionAgent`：关联 1 条根因诊断。
7. `UltraDynamicReviewer`：关联 1 条根因诊断。
8. `RoundArtifactRecorder`：关联 1 条根因诊断。

> 说明：本报告是规则化归因。它不会把相关性伪装成因果性；每条结论均保留触发指标，机制是否真正有效仍需同题消融或前后快照实验验证。
