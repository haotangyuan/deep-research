...
class ScopeAgentFrameworkContractTest {

    @ParameterizedTest
    @EnumSource(AgentFramework.class)
    void clarificationWorkflowIsRuntimeNeutral(AgentFramework framework) {
        ModelHandler modelHandler = modelHandlerWithResponses(framework,
        ...
        DeepResearchState state = stateWithUserMessage("research-contract-1", "Analyze AI");
        ...
        assertThat(eventPublisher.messages).contains("research-contract-1|assistant|Which market?");
    }

    @ParameterizedTest
    @EnumSource(AgentFramework.class)
    void researchBriefWorkflowIsRuntimeNeutral(AgentFramework framework) {
        ModelHandler modelHandler = modelHandlerWithResponses(framework,
                response("{\"needClarification\":false,\"question\":null,\"verification\":\"Ready\"}", 7, 3),
                response("{\"researchBrief\":\"Research AI adoption in healthcare\"}", 13, 8));
        RecordingEventPublisher eventPublisher = new RecordingEventPublisher();
        ScopeAgent scopeAgent = new ScopeAgent(modelHandler, new ObjectMapper(), eventPublisher);
        DeepResearchState state = stateWithUserMessage("research-contract-2", "Analyze AI adoption");
    ...
    private static ModelHandler modelHandlerWithResponses(AgentFramework framework, ResearchChatResponse... responses) {
        return new FakeModelHandler(framework, new FakeChatClient(responses));
    }
    ...
    private static final class FakeModelHandler extends ModelHandler {
        ...
        private FakeModelHandler(AgentFramework framework, ResearchChatClient chatClient) {
            super(new FakeRuntime(framework, chatClient));
    ...
    private static final class FakeRuntime implements AgentRuntime {
        ...
        private FakeRuntime(AgentFramework framework, ResearchChatClient chatClient) {
